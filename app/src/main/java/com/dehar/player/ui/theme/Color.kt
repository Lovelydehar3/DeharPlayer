package com.dehar.player.ui.theme

import androidx.compose.ui.graphics.Color

// Primary brand colors
val DeharOrange = Color(0xFFFF9800)
val DeharOrangeDark = Color(0xFFF57C00)
val DeharOrangeLight = Color(0xFFFFB74D)
val DeharOrangeVariant = Color(0xFFFFAB40)

// Background & Surface
val DeharBackground = Color(0xFF121212)
val DeharSurface = Color(0xFF1E1E1E)
val DeharSurfaceVariant = Color(0xFF2A2A2A)
val DeharSurfaceElevated = Color(0xFF333333)

// Text
val DeharOnBackground = Color(0xFFFFFFFF)
val DeharOnSurface = Color(0xFFE0E0E0)
val DeharOnSurfaceVariant = Color(0xFFAAAAAA)
val DeharOnPrimary = Color(0xFF000000)

// Status
val DeharError = Color(0xFFCF6679)
val DeharSuccess = Color(0xFF4CAF50)

// Player specific
val PlayerOverlay = Color(0x99000000)
val PlayerSeekBarPlayed = DeharOrange
val PlayerSeekBarBuffered = Color(0x80FFFFFF)
val PlayerSeekBarUnplayed = Color(0x33FFFFFF)
